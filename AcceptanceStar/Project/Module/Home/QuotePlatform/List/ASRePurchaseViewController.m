//
//  ASBuybackInformationViewController.m
//  AcceptanceStar
//
...
#import "ASRePurchaseViewController.h"
#import "YSCInfiniteLoopView.h"
#import "ASPopView.h"

#define keyOfCachedBanner               @"keyOfCachedBanner"

@interface ASRePurchaseViewController ()
...
@implementation ASRePurchaseViewController
...
- (void)viewDidLoad {
    ...
    self.asPopView = [ASPopView CreatePaymentView:PriceTypeRePurchase];
    ...
    addNObserver(@selector(refreshByNotification), kNotificationRefreshRePurchaseList);
}
...
- (NSString *)methodWithPath {
    return kResPathAppBondRepurchase;
}
...
- (Class)modelClassOfData {
    return ClassOfObject(BondRePurchaseModel);
}
- (NSString *)nibNameOfCell {
    return @"ASRePurchaseCell";
}
- (void)clickedCell:(id)object atIndexPath:(NSIndexPath *)indexPath {
    ...
    [self pushViewController:@"ASPriceDetailViewController"
                  withParams:@{kParamTitle : self.navigationItem.title,
                               ...
                               kParamPriceType : @(PriceTypeRePurchase),
                               kParamModel : object}];
}
