//
//  ASSaleInformationTableViewCell.m
//  AcceptanceStar
//
...
@implementation ASSellCell

+ (CGFloat)HeightOfCell {
    return AUTOLAYOUT_LENGTH(165);
}

- (void)layoutDataModel:(BondSellModel *)dataModel {
    NSString *iconName = @"";
    ...
    self.classLabel.text = dataModel.class_name;
    self.priceLabel.text = [NSString stringWithFormat:@"%@万元", FormatNumberValue(dataModel.price)];
    self.daysLabel.text = [NSString stringWithFormat:@"%ld天到期", dataModel.days];
//    self.rateLabel.text = [NSString stringWithFormat:@"报价利率：%@%%", dataModel.rate];
    self.cityLabel.text = [NSString stringWithFormat:@"%@", dataModel.city];
    //self.dateLabel.text = [YSCCommonUtils TimePassed:dataModel.date];
