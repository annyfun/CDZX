...
#import "ASQuotePriceViewController.h"
#import "YSCInfiniteLoopView.h"
#import "ASPopView.h"

#define keyOfCachedBanner               @"keyOfCachedBanner"

@interface ASQuotePriceViewController ()
...
@implementation ASQuotePriceViewController
...
- (void)viewDidLoad {
    ...
    self.asPopView = [ASPopView CreatePaymentView:PriceTypeQuotePrice];
    ...
    addNObserver(@selector(refreshByNotification), kNotificationRefreshQuotePriceList);
}
...
- (Class)modelClassOfData {
    return ClassOfObject(QuotePriceModel);
}
- (NSString *)nibNameOfCell {
    return @"ASQuotePriceCell";
}
- (void)clickedCell:(id)object atIndexPath:(NSIndexPath *)indexPath {
    ...
    [self pushViewController:@"ASPriceDetailViewController"
                  withParams:@{kParamTitle : self.navigationItem.title,
                               ...
                               kParamPriceType : @(PriceTypeQuotePrice),
                               kParamModel : object}];
}
@end
