...
@implementation ContentLeft

+ (CGFloat)heightWithItem:(TCMessage*)item {
    CGFloat height = 0.0;
    
    // 泡泡外围固定边距
    CGFloat offSetWidth  = kMessageLeftMarginsLeft + kMessageLeftMarginsRight;
    CGFloat offSetHeight = kMessageLeftMarginsTop + kMessageLeftMarginsBottom;
...
//    CGFloat pointX = kMessageLeftMarginsLeft;
//    CGFloat pointY = kMessageLeftMarginsTop;
//    CGFloat width = bounds.size.width - kMessageLeftMarginsLeft - kMessageLeftMarginsRight;
//    CGFloat height = bounds.size.height - kMessageLeftMarginsTop - kMessageLeftMarginsBottom;
//    self.frameContent = CGRectMake(pointX, pointY, width, height);
//    return self.frameContent;
//}

- (void)_init {
    UIImage* img = [[UIImage imageNamed:@"talk_bkg_msg_l"] stretchableImageWithLeftCapWidth:14 topCapHeight:14];
    [self setBackgroundImage:img forState:UIControlStateNormal];
    
    NSMutableArray* imgArr = [[NSMutableArray alloc] init];
    for (int i = 0; i < 3; i++) {
        UIImage* img = [UIImage imageNamed:[NSString stringWithFormat:@"talk_l_voice%d",i]];
