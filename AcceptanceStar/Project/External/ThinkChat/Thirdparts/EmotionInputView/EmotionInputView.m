...
- (void)generateInputView {
    UIScrollView * emojiScrollView = [[UIScrollView alloc] initWithFrame:self.bounds];
    emojiScrollView.pagingEnabled = YES;
    emojiScrollView.backgroundColor = [UIColor clearColor];
    [self addSubview:emojiScrollView];
    
//    int emojiRangeArray[] = {0xE001,0xE05A,0xE101,0xE15A,0xE201,0xE253,0xE301,0xE34D,0xE401,0xE44C,0xE501,0xE537};
    ...
    int countPerRow = 7;
    int rowPerPage = 4;
    int countPerPage = countPerRow * rowPerPage;
    int page = 0;
    ...
    for (int j = 0 ; j<28 ; j+=2 ) {
        ...
        for (unsigned short i = startIndex ; i<= endIndex ; i++ ) {
            ...
            page = indexCurrent / countPerPage;
            int indexPage = indexCurrent % countPerPage;
            int row = indexPage / countPerRow;
            int index = indexPage % countPerRow;
            
            UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
            button.frame = CGRectMake(page * emojiScrollView.frame.size.width + index * 45 + 5,
                                      row* 45 + 5,
                                      40,
                                      40);
            [button setTitle:[EmotionInputView emojiChar4To5:[NSString stringWithFormat:@"%C", i]] forState:UIControlStateNormal];
            [button setBackgroundImage:[UIImage imageWithColor:[UIColor orangeColor]] forState:UIControlStateHighlighted];
            button.tag = i;
            [button addTarget:self action:@selector(btnEmotionClick:)forControlEvents:UIControlEventTouchUpInside];
            [emojiScrollView addSubview:button];

        }
    }
    
    [emojiScrollView setContentSize:CGSizeMake(emojiScrollView.frame.size.width*(page+1), emojiScrollView.frame.size.height)];
}

- (void)btnEmotionClick:(UIButton*)sender {
