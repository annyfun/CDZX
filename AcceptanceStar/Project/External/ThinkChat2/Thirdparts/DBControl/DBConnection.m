...
+ (sqlite3*)openDatabase:(NSString*)dbFilename
{
    ...
    if (sqlite3_open([path UTF8String], &instance) != SQLITE_OK) {
        ...
        TCDemoLog(@"Failed to open database. (%s)", sqlite3_errmsg(instance));
        return nil;
    }        
    return instance;
}
...
+(void)dbVersionControl
{
    ...
    TCDemoLog(@"dbVersionControl before: %@ after: %@",version_old,version_new);
    
    if ( version_old == nil || ![version_new isEqualToString:version_old]) {
        TCDemoLog(@"del db file!!!");
        ...
        if ([fileManager fileExistsAtPath:writableDBPath]) {
            ...
            if (error) {
                TCDemoLog(@"Can not delete DB file with error : %@", [error localizedFailureReason]);
            }
        }
...
+ (void)deleteMessageCache
{
    ...
    if (sqlite3_exec(theDatabase, delete_message_cache_sql, NULL, NULL, &errmsg) != SQLITE_OK) {
        // ignore error
        TCDemoLog(@"Error: failed to cleanup chache (%s)", errmsg);
    }
}
...
+ (void)closeDatabase
{
    ...
    if (theDatabase) {
        ...
        TCDemoLog(@"launchCount %ld", (long)launchCount);
        if (launchCount-- <= 0) {
            TCDemoLog(@"Optimize database...");
