...
- (void)startDownload {
    if (imageURLString.length > 0 && [imageURLString hasPrefix:@"http"]) {
...
- (void)addOperation:(ImageProgress*)op {
    ...
    if (operation != nil &&
        ...
        operation.indexPath.row == op.indexPath.row &&
        operation.indexPath.section == op.indexPath.section) {
        exist = YES;
    } else {
        for (ImageProgress * opt in queue) {
            if (opt.tag == op.tag &&
                opt.indexPath.row == op.indexPath.row &&
                opt.indexPath.section == op.indexPath.section) {
...
- (void)imageProgress:(ImageProgress*)sender completed:(BOOL)bl {
    if (bl && sender.loaded) {
        ...
        } else if ([self.delegate respondsToSelector:@selector(imageProgressCompleted:indexPath:tag:)]) {
            UIImage * img = sender.image;
            [self.delegate imageProgressCompleted:img indexPath:sender.indexPath tag:sender.tag];
        }
    }
    [self executeNext];
}
