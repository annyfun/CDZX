...
@interface GroupNewController () <SingleTextFieldDelegate, SingleTextViewDelegate, UIImagePickerControllerDelegate, UIActionSheetDelegate, UINavigationControllerDelegate, TCResultGroupDelegate> {
...
@implementation GroupNewController
...
- (id)init
{
    self = [super initWithNibName:@"GroupNewController" bundle:nil];
    if (self) {
...
- (void)btnPressed:(id)sender {
    if (sender == btnCreate) {
        if (name && detail) {
            self.loading = YES;
            tagRequest = [[ThinkChat instance] createGroupName:name headImage:headImage description:detail extend:nil delegate:self];
        } else {
...
- (void)tcResultGroup:(TCGroup *)itemG error:(TCError *)itemE {
    self.loading = NO;
    if (itemE == nil) {
        [[AppDelegate instance] joinGroup:itemG];
        [KAlertView showType:KAlertTypeCheck text:@"创建成功" for:1.5 animated:YES];
