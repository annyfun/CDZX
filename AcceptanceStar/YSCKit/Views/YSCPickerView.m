        ...
        if (YSCPickerTypeAddress == self.pickerType) {
            ...
            if ([indexArray isKindOfClass:[NSArray class]]) {
                for (int i = 0; i < MIN(self.pickerView.numberOfComponents, [indexArray count]); i++) {
                    NSArray *tempArray1 = self.customDataArray[i];
                    NSInteger tempRow = MIN([tempArray1 count] - 1, [indexArray[i] integerValue]);
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    if (YSCPickerTypeAddress == self.pickerType) {
        ...
        NSArray *tempArray1 = self.customDataArray[component];
        return [tempArray1 count];
    }
    return 0;
}
