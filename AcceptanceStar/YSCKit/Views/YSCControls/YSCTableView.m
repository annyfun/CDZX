...
- (void)setEnableRefresh:(BOOL)enableRefresh {
    ...
    if (enableRefresh) {
        ...
        [self addLegendHeaderWithRefreshingBlock:nil];
    }
}
- (void)setEnableLoadMore:(BOOL)enableLoadMore {
    ...
    if (enableLoadMore) {
        ...
        [self addLegendFooterWithRefreshingBlock:nil];
    }
}
...
- (void)refreshAtPageIndex:(NSInteger)pageIndex response:(NSObject *)initObject error:(NSString *)errorMessage {
        ...
        if (errorMessage) {
            ...
            if ([newDataArray count] > 0) {
                ...
                for (NSObject *object in newDataArray) {
                    ...
                    //add new row
                    NSInteger rows = [weakSelf tableView:weakSelf numberOfRowsInSection:section];
                    if (NO == isPullToRefresh && rows > 0) {
                        [insertedIndexPaths addObject:[NSIndexPath indexPathForRow:row inSection:section]];
                    }
                }
                //--------------------------------------------
                
                if (NO == isPullToRefresh && isNotEmpty(insertedIndexPaths)) {//insert rows
    ...
    RequestFailure failureBlock = ^(NSInteger errorCode, NSString *errorMessage) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (self.numberOfCellBlock) {
        return self.numberOfCellBlock(section);
    }
    else {
