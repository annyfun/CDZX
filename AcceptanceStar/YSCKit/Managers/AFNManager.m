...
//            if (modelName == [CreateOrderModel class]) {
//                baseModel.stateInteger = 1 - baseModel.stateInteger;
//            }
     ...
     andImageDataParam:imageDataParam
      ...
      requestSuccessed:^(id responseObject) {
          ...
          if (1 == baseModel.stateInteger) {  //接口访问成功
              ...
              if (requestFailure) {
                  requestFailure(1102, baseModel.message);
              }
          }
...
+ (void)requestByUrl:(NSString *)url
  ...
  constructingBodyWithBlock: ^(id < AFMultipartFormData > formData) {
      for (NSString *imageName in imageDataParam.allKeys) {
          NSData *imageData = UIImagePNGRepresentation(imageDataParam[imageName]);
