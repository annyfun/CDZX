...
+ (void)checkNewVersion:(NewVersionModel *)versionModel showMessage:(BOOL)showMessage {
    if ([versionModel isKindOfClass:[NewVersionModel class]]) {
        BOOL isSkipTheVersion = [[NSUserDefaults standardUserDefaults] boolForKey:SkipVersion(versionModel.versionCode)];
        if ( ! isSkipTheVersion) {
                ...
                if ([NSString isNotEmpty:versionModel.downloadUrl]) {//TODO:这里可以进一步判断是否是标准的ios更新地址
                    ...
                    if (NO == versionModel.isForced ) {   //非强制更新的话才显示更多选项
                        [alertView bk_addButtonWithTitle:@"忽略此版本" handler:^{
                            [[NSUserDefaults standardUserDefaults] setBool:YES forKey:SkipVersion(versionModel.versionCode)];
...
#pragma mark - 分享相关设置
    ...
    [OpenShare connectWeixinWithAppId:AppKeyWeiXin];
    [OpenShare connectQQWithAppId:AppKeyQQ];
...
+ (NSString *)TimePassed2:(NSString *)timeStamp {
    NSDate *startDateTime = [NSDate dateFromTimeStamp:timeStamp];
    NSDate *nowDateTime = [NSDate date];
    
    return [startDateTime stringWithFormat:@"yyyy-MM-dd"];
}
